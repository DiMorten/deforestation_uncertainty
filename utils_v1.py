...
def resnet_block_spatial_dropout(x, n_filter, dropout_seed, ind):
    x_init = x

    ## Conv 1
    x = Conv2D(n_filter, (3, 3), activation='relu', padding="same", name = 'res1_net'+str(ind))(x)
    # x = Dropout(0.5, name = 'drop_net'+str(ind))(x, training = True)
    x = SpatialDropout2D(0.25, name = 'drop_net'+str(ind), seed = dropout_seed)(x, training = True)
...
def build_resunet_dropout_spatial(input_shape, nb_filters, n_classes, dropout_seed = None, last_activation='softmax'):
    ...
    res_block1 = resnet_block_spatial_dropout(input_layer, nb_filters[0], dropout_seed, 1)
    pool1 = MaxPool2D((2 , 2), name='pool_net1')(res_block1)
    
    res_block2 = resnet_block_spatial_dropout(pool1, nb_filters[1], dropout_seed, 2) 
    pool2 = MaxPool2D((2 , 2), name='pool_net2')(res_block2)
    
    res_block3 = resnet_block_spatial_dropout(pool2, nb_filters[2], dropout_seed, 3) 
    pool3 = MaxPool2D((2 , 2), name='pool_net3')(res_block3)
    
    res_block4 = resnet_block_spatial_dropout(pool3, nb_filters[2], dropout_seed, 4)
    
    res_block5 = resnet_block_spatial_dropout(res_block4, nb_filters[2], dropout_seed, 5)
    
    res_block6 = resnet_block_spatial_dropout(res_block5, nb_filters[2], dropout_seed, 6)
    
    upsample3 = Conv2D(nb_filters[2], (3 , 3), activation = 'relu', padding = 'same', 
                       name = 'upsampling_net3')(UpSampling2D(size = (2,2))(res_block6))
    
    upsample3 = SpatialDropout2D(dropout, seed = dropout_seed)(upsample3, training=True)
    ...
    upsample2 = SpatialDropout2D(dropout, seed = dropout_seed)(upsample2, training=True)
    ...
    upsample1 = SpatialDropout2D(dropout, seed = dropout_seed)(upsample1, training=True)
