...
- In Google Earth Engine, upload the image ROI (Region of interest) as a `.shp` file, and load it in the `polygon` variable. 
...
- Yearly deforestation between 2008 and 2022: Download "Incremento anual no desmatamento - Shapefile (2008/2022)" as a SHP file. Convert it to raster and crop its dimensions to the desired ROI using QGIS. Save the TIF file in `dataset/deforestation/{site}/deforestation_past_years.tif`, with site as `PA` or `MT`.
- Deforestation before 2008: Download "Máscara de área acumulada de supressão da vegetação nativa - Shapefile (2007)" as a SHP file. Convert it to raster and crop its dimensions to the desired ROI using QGIS. Save the TIF file in `dataset/deforestation/{site}/deforestation_before_2008_{site}.tif`, with site as `PA` or `MT`.
