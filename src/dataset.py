...
class ParaMultipleDates(ParaDeforestationTime):
	def __init__(self, dates=[2018,2019], addPastDeforestationInput = True):
		self.dates = dates
	...
	def addDeforestationTime(self, image_stack):
		...
		ic(image_stack.shape)
		deforestation_time = []
		for date in self.dates[:-1]: # if dates = [2018, 2019] then deforestation_time is from [2018]
			deforestation_time.append(
				np.load(self.paths.deforestation_time[date])
			)
		deforestation_time = np.concatenate(deforestation_time, axis=-1)
		image_stack = np.concatenate((deforestation_time, image_stack), axis=-1)
		del deforestation_time
	...
	def loadLabel(self):
		label_per_date = []
		for date in self.dates[1:]: # if dates = [2018, 2019] then label is from [2019]
			label_per_date.append(np.expand_dims(
				self.loadLabelFromDate(date), axis = -1))
